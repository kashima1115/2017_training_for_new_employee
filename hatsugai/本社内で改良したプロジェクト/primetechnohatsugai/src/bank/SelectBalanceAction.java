...
public class SelectBalanceAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping
		...
		Statement stmt = null;
		ResultSet rs = null;
		String balance="";//残高を入れるString型変数balance
		try{
			// DB接続
			con = source.getConnection();
			// ステートメント生成
			stmt = con.createStatement();
			//ログイン時に取得した口座番号を元に残高を取得
			String sqlstr="SELECT account_balance FROM user WHERE account_no";
			sqlstr+="='"+frm.getAccountNumber()+ "'";
			rs=stmt.executeQuery(sqlstr);
			while(rs.next()){
				balance=rs.getString(1);//残高をbalanceに代入
			}
			frm.setBalance(balance);//formに残高をセット
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			if(rs != null){
		...
		// 次の画面遷移のためのパラメータ設定
       return mapping.findForward("balance");//残高照会画面に遷移
   }
}
